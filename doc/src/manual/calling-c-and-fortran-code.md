...
mutable objects which are normally heap-allocated, but also to scalar values
such as integers and floats which are normally stack-allocated and passed in
registers in C and Julia.
...
pointers to heap-allocated values. The return type should either be `Cvoid` for
